...
from models.DINetdepth import DINet
...
if __name__ == "__main__":
    ...
    training_data_loader = DataLoader(dataset=train_data,  batch_size=opt.batch_size, shuffle=True,num_workers=min(72, os.cpu_count()),pin_memory=True,drop_last=True)
    ...
    depth_encoder = depth_encoder.to(device).eval()
    depth_decoder = depth_decoder.to(device).eval()
    for param in chain(depth_encoder.parameters(), depth_decoder.parameters()):
        param.requires_grad_(False)

    # init network
    net_g = DINet(opt.source_channel,opt.ref_channel,opt.audio_channel,depth_encoder,depth_decoder)
    ...
    # 解冻深度模块参数
    # for param in net_g.module.depth_encoder.parameters():
    #     param.requires_grad = True
    # for param in net_g.module.depth_decoder.parameters():
    #     param.requires_grad = True

    # # 并行化优化
    # if torch.cuda.device_count() > 1:
    #     print(f"Using {torch.cuda.device_count()} GPUs!")
    #     net_g = convert_model(nn.DataParallel(net_g)).to(device)
    #     net_dI = nn.DataParallel(net_dI).to(device)
    #     net_vgg = nn.DataParallel(net_vgg).to(device)
    #net_g = nn.DataParallel(net_g, device_ids=device_ids).cuda()
    #net_g = convert_model(net_g)  # 同步批归一化要在DataParallel之后
    #net_dI = nn.DataParallel(net_dI, device_ids=device_ids).cuda()
    #net_vgg = nn.DataParallel(Vgg19(), device_ids=device_ids).cuda()
