...
if __name__ == '__main__':
    ...
    print('extracting whisper feature from : {}'.format(opt.driving_audio_path))
    whisper_model = load_model("tiny")  # 你可以选择 "tiny", "base", "small", "medium", "large" 模型
    if not os.path.exists(opt.driving_audio_path):
        raise ('wrong audio path :{}'.format(opt.driving_audio_path))

    # 加载音频并转换为 log-Mel spectrogram
    import whisper

    audio = whisper.load_audio(opt.driving_audio_path)
    mel = whisper.log_mel_spectrogram(audio).numpy()

    # 适配 DeepSpeech 的 ds_feature 形状
    res_frame_length = mel.shape[1]  # whisper 的时间步数
    ds_feature_padding = np.pad(mel.T, ((2, 2), (0, 0)), mode='edge')  # 适配原代码的 padding 方式
    ...
    video_landmark_data = load_landmark_openface(opt.source_openface_landmark_path).astype(int)  #1111111111111111
