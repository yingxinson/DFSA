...
if __name__ == "__main__":
    ...
    training_data_loader = DataLoader(dataset=train_data, batch_size=opt.batch_size, shuffle=True,
                                      num_workers=min(72, os.cpu_count()), pin_memory=True, drop_last=True)
    ...
    checkpoint = torch.load(opt.pretrained_frame_DFSA_path)
        ...
        for iteration, data in enumerate(training_data_loader):
            ...
            print(
                "===> Epoch[{}]({}/{}):  Loss_DI: {:.4f} Loss_GI: {:.4f} Loss_DV: {:.4f} Loss_GV: {:.4f} Loss_perception: {:.4f} Loss_sync: {:.4f} lr_g = {:.7f} ".format(
                    epoch, iteration, len(training_data_loader), float(loss_dI), float(loss_g_dI), float(loss_dV),
                    float(loss_g_dV), float(loss_g_perception), float(loss_sync),
