...
class DeepSpeech():
    ...
    def compute_audio_feature(self,audio_path):
        ...
        #audio, audio_sample_rate = sf.read(audio_path)
        if audio.ndim != 1:
            warnings.warn(
        ...
        if audio_sample_rate != self.target_sample_rate:
            resampled_audio = resampy.resample(
                x=audio.astype(float),   #11111111111111111111111111111111111111111111111111
                #x=audio.astype(float),
                sr_orig=audio_sample_rate,
                sr_new=self.target_sample_rate)
        else:
            resampled_audio = audio.astype(float)  #11111111111111111111111111111111111111111
