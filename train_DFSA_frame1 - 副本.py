...
from models.DFSAs1 import DFSA
...
from torch.cuda.amp import GradScaler, autocast  # === AMP: 新增导入 ===
...
if __name__ == "__main__":
    ...
    # init network
    net_g = DFSA(opt.source_channel, opt.ref_channel, opt.audio_channel,d_model=256,nhead=8,num_layers=1)
    ...
    for epoch in range(opt.start_epoch, opt.non_decay + opt.decay + 1):
        net_g.train()
        net_dI.train()
        net_vgg.eval()  # 一般 VGG 做感知损失用 eval 更稳定（可选）
        ...
        for iteration, data in enumerate(training_data_loader):
            ...
            # === 关键变化1：D阶段用 no_grad 跑一次 G，仅供 D 使用 ===
            with torch.no_grad():
                with autocast():  # 这里也可以用 AMP 加速 G 的前向
                    fake_out_for_d = net_g(source_image_mask, reference_clip_data, deepspeech_feature)

            # === AMP + detach：只给 D 传假图，不保留 G 的计算图 ===
            with autocast():
            ...
            scaler_dI.scale(loss_dI).backward()   # 不再使用 retain_graph=True
            ...
            # === 关键变化2：G阶段单独再前向一次 net_g（带梯度） + AMP ===
            with autocast():
